...
```mermaid
flowchart TD
  T[Текст запроса → POST /api/parse] --> A
  ...
  D --> F[Не прошедшие: все причины в excluded + воронка funnel]
  E --> G[До 3 карточек: сравнительные объяснения по фактам]
  G --> L{Есть OPENAI_API_KEY?}
  L -- да --> M[LLM редактирует тексты → проверка → кэш]
...
| `POST /api/match` | Подбор по структурированным параметрам |
| `POST /api/parse` | Текст → параметры формы, фрагмент-источник каждого поля, недостающие поля |
| `POST /api/ask` | Текст → разбор и сразу подбор, если хватает обязательных параметров |
...
4. Поменять только дату на **12.12.2026**: выдача меняется, кандидаты из октябрьской
   выдачи исключены с причиной «занят 12 декабря» (сравнение дат есть в интерфейсе).
5. Ввести текстом: «Нужен ведущий на свадьбу 14 ноября в Алматы, бюджет до 800 тысяч» —
   поля формы заполняются, видно, из какого фрагмента взято каждое.
...
несовпадение исходов или недоступность API — ненулевой код.
Подробные входы и фактические ответы: [docs/DEMO.md](docs/DEMO.md).

7. Тесты (без сети; LLM-тесты подменяют SDK):
