...
Рейтинг кандидатов, прошедших фильтры, — взвешенная сумма оценок факторов из `score_parts`, округлённая до 6 знаков.
При равенстве порядок определяется по `id`. Рейтинг не является вероятностью.

Оценки в `score_parts` показаны **до** применения весов. Для профилей, у которых есть вектор,
