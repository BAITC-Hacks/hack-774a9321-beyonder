...
![Ключ API не нужен](https://img.shields.io/badge/%D0%BA%D0%BB%D1%8E%D1%87_API-%D0%BD%D0%B5_%D0%BD%D1%83%D0%B6%D0%B5%D0%BD-2E7D32)
![HackAlem AI · трек 06](https://img.shields.io/badge/HackAlem_AI-%D1%82%D1%80%D0%B5%D0%BA_06_%C2%B7_%D0%9A%D1%80%D0%B5%D0%B0%D1%82%D0%B8%D0%B2%D0%BD%D1%8B%D0%B5_%D0%B8%D0%BD%D0%B4%D1%83%D1%81%D1%82%D1%80%D0%B8%D0%B8-173C35)
...
| **Прозрачный рейтинг** | «Как посчитан рейтинг»: вклад бюджета, описания, часов, языка и качества данных |
...
| :-: | --- | --- |
| 1 | Нажать **«Фотографы в Алматы»** | Исход «Нашли три подходящих варианта». Три карточки с одинаковыми фактами (цена и % бюджета, свободен, часы, языки), своей строкой «Чем выделяется» и своим объяснением. Раскрывающийся блок «Как посчитан рейтинг». Время ответа — сотые доли секунды. Метка «Объяснение по данным» |
...
| 4 | В поле «Опишите мероприятие своими словами» вставить<br>*Нужен ведущий на казахскую свадьбу 14 ноября в Алматы, бюджет до 800 тысяч, часов на 6*<br>→ «Разобрать» → «Подобрать подрядчиков» | Форма заполнена, у каждого поля — фрагмент текста; совет выбрать язык «казахский» или формат «той». Подбор сам не запускается. После нажатия — один вариант: Мицури Канроджи |
...
Рейтинг прошедших фильтры — взвешенная сумма, округлённая до 6 знаков; при равенстве порядок по `id`. Это не вероятность.

| Фактор | Вес | Что означает |
| --- | :-: | --- |
| `budget` | 0.40 | Запас бюджета относительно цены «от» |
| `relevance` | 0.30 | Маркеры формата в описании и узкая специализация |
...
| `POST /api/match` | город, дата, формат, категория, бюджет; по желанию часы и язык | `outcome`, `message`, `cards` (с `highlights`, `score_parts`, `explanation_source`), `excluded`, `hints`, `funnel` |
...
- **Смысл описания** учитывается через маркеры формата в описании (фактор «описание и формат», 30%) и дословные цитаты
  в объяснениях. Семантический поиск по эмбеддингам — следующий шаг, см. [развитие](#масштабирование-и-развитие).
...
Ожидается `PASS: 8 ответов, 0 ошибок` и `Источник объяснения: llm`. Ответы модели сохраняются в `.cache/explanations.json`
и переживают перезапуск: кнопки-сценарии, сравнение дат и пример с фразой затем отвечают из кэша. Запросы, которых не было
...
- **Семантический поиск** — эмбеддинги описаний, чтобы находить подрядчиков по смыслу («камерная свадьба на природе»).
