...
Рейтинг прошедших фильтры — взвешенная сумма оценок факторов из `score_parts`, округлённая до 6 знаков;
при равенстве порядок по `id`. Это не вероятность. Оценки в `score_parts` показаны **до** применения весов;
Для профилей с вектором `semantic` дополнительно показывает близость, используемую как `relevance`, и отдельно
