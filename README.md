...
Нужны Git и Python 3.10+. Выполните команды в терминале; после установки сервер останется запущенным.
...
Откройте `http://127.0.0.1:8000` и нажмите «Фотографы в Алматы». Во втором терминале **из корня клона**
запустите `.\.venv\Scripts\python.exe scripts/demo.py` (macOS/Linux: `.venv/bin/python scripts/demo.py`).
Ожидается `PASS: 8 ответов, 0 ошибок`. Время установки зависит от сети.

### Результат без ключа

Форма, воронка и три карточки с объяснениями по данным после нажатия «Фотографы в Алматы»:

![Интерфейс: воронка отбора и три карточки с объяснениями по данным](docs/screenshot.png)
...
(«Дороже самого доступного на 50 000 ₸», «До 12 ч — дольше остальных»). `score_parts` содержит
оценки факторов от 0 до 1 до применения весов; итоговый `score` — их взвешенная сумма.
`explanation_source` показывает, как собрано объяснение: `template` — по правилам из фактов профиля, `llm` — отредактировано моделью
...
3. Ожидается `found`: три карточки с одинаковой структурой фактов (цена и % бюджета, свободен,
   ...
   блок «Как посчитан рейтинг» показывает оценки факторов и веса; рядом с заголовком — время ответа.
...
```
   ...
   Ожидается `PASS: 8 ответов, 0 ошибок`. При успешном ответе модели у карточки будет
   `Источник объяснения: llm`; при таймауте или отклонении ответа остаётся `template`.
   Успешные AI-ответы сохраняются в `.cache/explanations.json` и переживают перезапуск сервера.
   Если AI-ответ для них получен при прогреве, точно совпадающие запросы кнопок-сценариев,
   сравнения дат и примера с фразой используют кэш. Новые условия обращаются к модели
   на лету с ограничением 6 с и тем же резервным режимом.
...
| Прозрачный рейтинг: «Как посчитан рейтинг» показывает оценки бюджета, описания, часов, языка и качества данных до взвешивания и их веса | Жюри и пользователь видят, почему порядок именно такой; рейтинг не выдаётся за вероятность | `score_parts` в карточке, интерфейс |
