...
### Демо с AI-объяснениями

Без ключа сервис полностью работает: карточки помечены «Объяснение по данным», сценарий тот же.
С ключом модель редактирует текст объяснений по проверенным фактам; отбор и порядок карточек не меняются.
...
3. Прогреть кэш перед показом — во втором терминале прогнать демо-сценарии:
...
```
   ...
   сохраняются в `.cache/explanations.json` и переживают перезапуск сервера, поэтому на показе
   кнопки-сценарии, сравнение дат и пример с фразой отвечают из кэша, мгновенно. Запросы, которых
   не было при прогреве (например, «Применить» или «Другие даты»), обращаются к модели на лету — до 6 с.
4. В интерфейсе у карточек появится метка «AI-объяснение» вместо «Объяснение по данным».
   Если ключа нет, модель недоступна, не уложилась в 6 с или ответ не прошёл проверку фактов —
   карточка остаётся с объяснением по данным, и это видно по метке. Причины отклонения
   ответа модели можно увидеть в консоли сервера, запустив его с `LLM_DEBUG=1` в `.env`.
