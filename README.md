...
> [!NOTE]
> **Ключ API не нужен.** Без ключа работает всё: подбор, рейтинг со смысловой оценкой описаний (эмбеддинги уже
> сохранены в `data/embeddings.json`), «План Б», разбор фразы. Объяснения собираются
> по данным каталога (метка «Объяснение по данным»), и все ожидаемые результаты ниже приведены именно для такого запуска.
...
**Или в Docker** — одной командой, без установки Python (нужен Docker с Compose):

```bash
docker compose up --build
```

Интерфейс откроется на http://localhost:8000; другой порт — `PORT=8001 docker compose up --build`.
Ключ API не нужен; при желании его можно передать через переменную окружения `OPENAI_API_KEY`.
...
| :-: | --- | --- |
| 1 | Нажать **«Фотографы в Алматы»** | Исход «Нашли три подходящих варианта»: Леорио Паради, Альфонс Элрик, Нобара Кугисаки. Три карточки с одинаковыми фактами (цена и % бюджета, свободен, часы, языки), своей строкой «Чем выделяется» и своим объяснением. Раскрывающийся блок «Как посчитан рейтинг» с оценками факторов и весами. Время ответа — сотые доли секунды. Метка «Объяснение по данным» |
...
| 3 | Нажать **«Сравнить даты»** | Подборки на 10 октября и 12 декабря рядом. 12 декабря подходит только Сацуки Кусакабэ, октябрьская тройка отмечена «занят 12 декабря»; под декабрьской — «Другие даты»: 13 декабря (вс) и 7 декабря (пн), по 3 варианта |
...
| Ответ до 10 секунд | Время ответа над подборкой; LLM-слой ограничен 6 с | `tests/test_explain_llm.py` |
...
**Смысловая релевантность:** эмбеддинги OpenAI `text-embedding-3-small` для каждого предложения описаний посчитаны заранее (`scripts/build_embeddings.py`) и лежат в `data/embeddings.json` — подбор работает без сети и ключа; ключ нужен только чтобы пересчитать их.
...
| `app/semantic.py` | Смысловая близость предложений описания к формату по заранее посчитанным эмбеддингам из `data/embeddings.json`; без сети |
...
Рейтинг прошедших фильтры — взвешенная сумма оценок факторов (каждая от 0 до 1), округлённая до 6 знаков;
при равенстве порядок по `id`. Это не вероятность. В `score_parts` оценки показаны до весов и округлены до 3 знаков,
поэтому пересчёт по JSON может немного отличаться от `score`; `semantic` повторяет `relevance` для наглядности
и второй раз в сумму не входит.

| Фактор | Вес | Что означает |
| --- | :-: | --- |
| `budget` | 0.40 | Запас бюджета относительно цены «от» |
| `relevance` | 0.30 | Смысловая близость описания к формату: косинус лучшего предложения описания к описанию формата (`score_parts.semantic`) |
...
| `POST /api/match` | город, дата, формат, категория, бюджет; по желанию часы и язык | `outcome`, `message`, `cards` (с `highlights`, `score_parts` — оценки факторов от 0 до 1 до весов, `explanation_source`), `excluded`, `hints`, `funnel` |
...
- **Смысл описания** учитывается через эмбеддинги: фактор «описание и формат» (30%) — это смысловая близость предложений
  описания к формату, посчитанная заранее, поэтому ранжирование остаётся детерминированным и работает без сети.
  Объяснение цитирует самое близкое предложение дословно.
...
| **Смысловая релевантность без сети**: эмбеддинги описаний посчитаны заранее, во время подбора — только косинус; порядок детерминирован и без ключа | `test_semantic_order_is_deterministic_without_key_or_network` |
...
| AI (необязательно) | OpenAI Python SDK — OpenAI или NVIDIA API Catalog через OpenAI-совместимый endpoint |
| Смысловая релевантность | OpenAI `text-embedding-3-small`, посчитанные заранее и сохранённые в `data/embeddings.json`; во время работы — только косинус, без сети |
| Данные | CSV каталога организаторов, ответы API в JSON |
| Запуск | Uvicorn локально или Docker Compose (`Dockerfile`, `docker-compose.yml`) |
...
необязательный LLM-слой и пересчёт эмбеддингов (`scripts/build_embeddings.py`), при подборе не используется; `python-dotenv` — чтение `.env` через
`--env-file`; `pytest`, `httpx` — тесты. Интернет нужен только для установки и для LLM-слоя.
...
| `OPENAI_API_KEY` | пусто | Ключ OpenAI (platform.openai.com → API keys) — для редактуры объяснений и пересчёта эмбеддингов `scripts/build_embeddings.py` |
...
- **Поиск по смыслу запроса** — эмбеддинги описаний уже есть; следующий шаг — сравнивать с ними свободный текст заказчика
  («камерная свадьба на природе»), а не только формат мероприятия.
