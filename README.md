...
> [!NOTE]
> **Ключ API не нужен.** Без ключа работает всё: подбор, рейтинг, «План Б», разбор фразы. Объяснения собираются
...
Рейтинг прошедших фильтры — взвешенная сумма оценок факторов (каждая от 0 до 1, в ответе — `score_parts`), округлённая
до 6 знаков; при равенстве порядок по `id`. Это не вероятность.
